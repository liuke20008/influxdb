...
func TestGroupSeriesCursor_Next(t *testing.T) {
	tests := []struct {
		name string
		cur  seriesCursor
		keys []string
		...
		{
			name: "sort tag1 in partial series",
			cur: &sliceSeriesCursor{
				rows: newSeriesRows(
					"aaa,tag0=val0",
					"aaa,tag0=val1",
		...
		{
			name: "sort tag2,tag1 with partial series",
			cur: &sliceSeriesCursor{
				rows: newSeriesRows(
					"aaa,tag0=val0",
					"aaa,tag0=val1",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cur := newGroupResultSet(context.Background(), tt.cur, tt.keys)

			var rows []seriesRow

			for i := range cur.rows {
				rows = append(rows, *cur.rows[i].row)
			}
